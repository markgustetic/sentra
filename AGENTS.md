...
- `sentra setup` is the guided first-run surface. It may invoke AWS CLI SSO
  login, write config, prepare AWS S3 bucket settings, and initialize the repo,
  but must never write secret material to `sentra.yaml` or logs.
