...
On macOS this writes a LaunchAgent under `~/Library/LaunchAgents` and loads it
with `launchctl bootstrap gui/$UID …`. On Linux it writes systemd user
service/timer files under `~/.config/systemd/user` and runs `systemctl --user
enable --now sentra-demo.timer`. Either way the timer is live immediately —
no re-login. If the OS cannot be reached (a headless SSH session, say), the
files stay written and both `install` and `status` print the command to run
later. Sentra does not install a background daemon; the scheduler launches
`sentra policy run demo --if-due` with your existing config and passphrase
resolution. A slot missed while the machine was asleep fires on wake; one
missed while it was shut down is caught up at your next login, and a slot
