...
// parseRecommendations decodes the model's final text as a JSON array
// of Recommendation. Strips a leading code-fence marker if present —
// some models wrap their JSON in ```json fences even when told not to.
// Empty arrays are valid (and correct: "nothing to recommend").
func parseRecommendations(text string) ([]Recommendation, error) {
	// Strip optional ```json ... ``` fences. We tolerate a single fence
	// because the cost is low and it makes the orchestrator more
	// forgiving against minor format drift.
	t := strings.TrimSpace(text)
	if strings.HasPrefix(t, "```") {
		// Find the first newline after the opening fence and the last
		// closing fence; everything in between is the payload.
