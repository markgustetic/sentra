...
// The wizard's blink wiring. It takes the same exception the palette and the
// modal prompts do — blink only, no ui.FieldBox — because every one of its
// input rows ALREADY carries a glyph focus affordance: detailRow and passRow
// prepend ui.SelectRow's ▍, and the backup-user profile row does the same.
// A box would be a second marker, and a 3-line block cannot be concatenated
// onto a single "label ▍ input" row without destroying that layout. What the
// wizard was missing is the blink itself: it returned no focus cmd anywhere,
// so every cursor in it sat static in a live terminal.

// TestSetupWizard_EnteringDetailsSchedulesBlink: the wizard lands on the
// fieldless backend stage, so Init has nothing to blink; committing the
// backend is the bucket field's first focus and must carry the cmd.
// Existence-only: the fields are built inside NewSetupWizardView, so there
// is no handle to drop BlinkSpeed on before that constructor's Focus() runs.
func TestSetupWizard_EnteringDetailsSchedulesBlink(t *testing.T) {
	t.Setenv("SENTRA_PASSPHRASE", "")
	v := NewSetupWizardView(Deps{Config: &config.Config{}})
	if v.Init() != nil {
		t.Fatal("Init must not blink: the backend stage focuses no field")
	}
	m, _ := v.Update(tea.WindowSizeMsg{Width: 80, Height: 24})
	v = m.(SetupWizardView)
	_, cmd := v.Update(tea.KeyMsg{Type: tea.KeyEnter})
	if cmd == nil {
		t.Fatal("committing the backend must schedule the bucket field's blink")
	}
}

// TestSetupWizard_DetailsTabSchedulesBlinkButNotOnTheIAMToggle: tab onto a
// text field (re)starts its blink; tab onto the IAM pseudo-field focuses
// nothing, so there is no blink to start.
func TestSetupWizard_DetailsTabSchedulesBlinkButNotOnTheIAMToggle(t *testing.T) {
	v := setupAtDetails(t, 0) // AWS: 4 fields + the IAM toggle
	// The fields already exist, so BlinkSpeed can be dropped before tab
	// fires — the handler's cmd is the REAL one Focus() produces, and
	// executing it (assertBlinkCmd does) would otherwise block for ~530ms.
	for i := range v.fields {
		v.fields[i].Cursor.BlinkSpeed = time.Millisecond
	}
	m, cmd := v.Update(tea.KeyMsg{Type: tea.KeyTab}) // bucket -> prefix
	v = m.(SetupWizardView)
	assertBlinkCmd(t, cmd)

	for v.fieldCursor != v.detailFieldCount() { // walk onto the IAM toggle
		m, cmd = v.Update(tea.KeyMsg{Type: tea.KeyTab})
		v = m.(SetupWizardView)
	}
	if cmd != nil {
		t.Fatal("tab onto the IAM toggle must not schedule a blink — no field is focused")
	}
}

// TestSetupWizard_RoutesBlinkTicksPerStage: a tick must reach whichever
// input the current stage has focused, on each of the three stages that
// have one. A bare cursor.BlinkMsg{} won't do — bubbles/cursor rejects a
// tag that doesn't match its internal counter, which Focus() has already
// advanced — so each case mints a tag-matched tick from its own field.
func TestSetupWizard_RoutesBlinkTicksPerStage(t *testing.T) {
	t.Run("details", func(t *testing.T) {
		v := setupAtDetails(t, 0)
		v.fields[setupFieldBucket].Cursor.BlinkSpeed = time.Millisecond
		tick := v.fields[setupFieldBucket].Cursor.BlinkCmd()
		if _, cmd := v.Update(tick()); cmd == nil {
			t.Fatal("tick not routed to the focused bucket field")
		}
	})
	t.Run("passphrase", func(t *testing.T) {
		...
		if _, cmd := v.Update(tick()); cmd == nil {
			t.Fatal("tick not routed to the focused newPass field")
		}
	})
	t.Run("actions profile row", func(t *testing.T) {
		...
		if _, cmd := v.Update(tick()); cmd == nil {
			t.Fatal("tick not routed to the focused profile input")
		}
	})
	t.Run("backend stage has no field", func(t *testing.T) {
		t.Setenv("SENTRA_PASSPHRASE", "")
		v := NewSetupWizardView(Deps{Config: &config.Config{}})
		if _, cmd := v.Update(cursor.BlinkMsg{}); cmd != nil {
			t.Fatal("a tick on the fieldless backend stage must be a no-op")
		}
	})
}

// TestSetupWizard_PassphraseFocusTransitionsSchedulesBlink covers both the
// stage's entry (newPass's first focus) and its tab cycle.
func TestSetupWizard_PassphraseFocusTransitionsSchedulesBlink(t *testing.T) {
	v := setupAtActions(t)
	m, entryCmd := v.Update(tea.KeyMsg{Type: tea.KeyEnter}) // -> stagePassphrase
	v = m.(SetupWizardView)
	if v.stage != stagePassphrase {
		t.Fatalf("stage = %v, want stagePassphrase", v.stage)
	}
	if entryCmd == nil {
		t.Fatal("landing on the passphrase stage must schedule newPass's blink")
	}

	v.confirmPass.Cursor.BlinkSpeed = time.Millisecond
	_, tabCmd := v.Update(tea.KeyMsg{Type: tea.KeyTab})
	assertBlinkCmd(t, tabCmd)
}

// TestSetupWizard_ProfileRowFocusSchedulesBlink: arrowing onto the
// backup-user profile row focuses its input, so that move carries the
// blink; arrowing off it blurs the input and carries none.
func TestSetupWizard_ProfileRowFocusSchedulesBlink(t *testing.T) {
	v := setupAtActions(t)
	v.backupProfile.Cursor.BlinkSpeed = time.Millisecond
	...
	if m.(SetupWizardView).actionCursor == actionRowProfile {
		t.Fatal("precondition: the cursor should have left the profile row")
	}
	if offCmd != nil {
