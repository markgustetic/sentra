...
// TestJobs_ExactlyOneBoxAndItFollowsFocus is the jobs form's inheritance of
// the affordance the deleted PoliciesView carried: the box marks exactly one
// field at a time and follows focus across tab. The delta over a
// fully-blurred baseline proves it tracks focus, not a fixed field position.
//
// The form only exists once the operator presses 'a' — that first-focus
// keypress (newPolicyForm's name.Focus()) is this view's activation path, so
// its returned cmd must schedule the blink. newPolicyForm builds `name` fresh
// inside the same call 'a' triggers, so there is no pre-existing field handle
// to drop BlinkSpeed on before it runs; entryCmd is checked for existence
// only (executing it would block for the default ~530ms).
// TestBlinkChain_ClosesEndToEnd (snapshots_test.go) proves the real
// round-trip once, on a key-triggered site where BlinkSpeed CAN be preset.
func TestJobs_ExactlyOneBoxAndItFollowsFocus(t *testing.T) {
	deps, _ := jobsDeps(t)
	v := newJobsForTest(t, deps)
	...
	v = m.(JobsView)
	if v.stage != jobsForm {
		t.Fatalf("stage = %v, want jobsForm", v.stage)
	}
	if entryCmd == nil {
		t.Fatal("expected a blink command from 'a', got nil")
	}
	...
	// v.form.path already exists (the form was built by 'a' above), so its
	// BlinkSpeed can be dropped before tab fires — the tab handler's cmd is
	// the REAL one Focus() produces, and executing it (assertBlinkCmd does)
	// would otherwise block for the default ~530ms.
...
// TestJobs_NoBoxWhenToggleFocused: the tab cycle visits two non-text steps
// (check, prune) after the four text fields. No field is focused there, so
// the box disappears entirely and there is no blink to restart — the rule
// the deleted policies form proved, carried over.
func TestJobs_NoBoxWhenToggleFocused(t *testing.T) {
	deps, _ := jobsDeps(t)
	v := newJobsForTest(t, deps)
	...
	for i := 0; i < 4; i++ { // name -> path -> tags -> schedule -> check
		var mm tea.Model
		mm, cmd = v.Update(tea.KeyMsg{Type: tea.KeyTab})
		v = mm.(JobsView)
	}
	if v.form.focus != 4 {
		t.Fatalf("focus after four tabs = %d, want 4 (the check toggle)", v.form.focus)
	}
	if got := boxCount(v.View()); got != n {
		t.Fatalf("toggle focused: boxCount = %d, want %d (no field box at all)", got, n)
	}
	if cmd != nil {
		t.Fatal("tab onto a toggle must not schedule a blink — no field is focused")
	}
}

// TestJobs_EditFormSchedulesBlink: 'e' lands on paths (the name is
// read-only in edit mode), so that keypress is paths' first focus and must
// carry the blink. Existence-only for the same reason as 'a':
// prefilledPolicyForm builds the field inside the call the key triggers.
func TestJobs_EditFormSchedulesBlink(t *testing.T) {
	deps, _ := jobsDeps(t)
	v := newJobsForTest(t, deps)
	m, cmd := v.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'e'}})
	if m.(JobsView).stage != jobsForm {
		t.Fatalf("stage = %v, want jobsForm", m.(JobsView).stage)
	}
	if cmd == nil {
		t.Fatal("expected a blink command from 'e', got nil")
	}
}

// TestJobs_RoutesBlinkTicksWhileFormFocused: blink ticks must reach the
// focused form field so the schedule continues. A bare cursor.BlinkMsg{}
// won't do — bubbles/cursor tags each scheduled tick and rejects one whose
// tag doesn't match its current count (stale-tick guard), and Focus()
// already advanced that counter past zero — so the test captures a
// genuinely tag-matched tick from the field's own cursor.
func TestJobs_RoutesBlinkTicksWhileFormFocused(t *testing.T) {
	deps, _ := jobsDeps(t)
	v := newJobsForTest(t, deps)
	m, _ := v.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'a'}})
	v = m.(JobsView)
	...
	if _, cmd := v.Update(tick()); cmd == nil {
		t.Fatal("blink tick was not routed to the focused name field")
	}
}

// TestJobs_IgnoresBlinkTicksOutsideTheForm: the list stage has no text
// field, so a tick there must be a no-op rather than reaching a blurred
// field and restarting a chain nothing is showing.
func TestJobs_IgnoresBlinkTicksOutsideTheForm(t *testing.T) {
	deps, _ := jobsDeps(t)
	v := newJobsForTest(t, deps)
	if v.stage != jobsList {
